...
class UpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'key' => [
                'required', 'string', 'alpha_num', 'max:255', Rule::unique(ModuleColumn::class)->where('module_id', $this->route('module')->id)->ignore($this->route('moduleColumn')->id)
            ],
            'name' => [
                ...
                'required', new EnumValue(ModuleColumnType::class, false), function ($attribute, $value, $throw) {
                    if (ModuleColumnType::_FILE == $value) {
                        ...
                        if ($module->type->is(\App\Enums\ModuleType::GALLERY)) {
                            if ($module->moduleColumns()->where(['type' => ModuleColumnType::_FILE])->whereNot('id', $this->route('moduleColumn')->id)->count()) {
