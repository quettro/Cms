...
class UpdateRequest extends FormRequest
{
    ...
    {
        return [
            'web_page_language_id' => [
                'required', 'numeric', Rule::exists(WebPageLanguage::class, 'id')->where('web_page_id', $this->route('webPage')->id)
            ],
            ...
            'webpage.route' => [
                'required', 'max:255', 'regex:/^' . WebPage::$regexTheRouteForValidation . '$/', Rule::unique(WebPage::class, 'route')->where('web_site_id', $this->route('webSite')->id)->where('parent_id', $this->input('webpage.parent_id'))->ignore($this->route('webPage')->id)
            ],
            'webpage.parent_id' => [
